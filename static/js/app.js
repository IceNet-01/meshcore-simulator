...
function initEventListeners() {
    ...
    // Clear route cache (MeshCore)
    const clearRoutesBtn = document.getElementById('btn-clear-routes');
    if (clearRoutesBtn) {
        clearRoutesBtn.addEventListener('click', () => {
            fetch('/api/routes/clear', { method: 'POST' })
                .then(response => response.json())
                .then(data => {
                    log('Route cache cleared', 'info');
                    updateRouteCacheDisplay(0);
                })
                .catch(err => log('Failed to clear routes: ' + err, 'error'));
        });
    }
...
function drawNode(node) {
    ...
    // Role indicator for repeaters/room servers (MeshCore) or routers (Meshtastic)
    if (node.role === 'ROUTER' || node.role === 'REPEATER' || node.role === 'ROOM_SERVER') {
        ctx.fillStyle = '#ffffff';
        ctx.font = 'bold 10px sans-serif';
        let roleLabel = 'R';
        if (node.role === 'REPEATER') roleLabel = 'P';
        if (node.role === 'ROOM_SERVER') roleLabel = 'S';
        ctx.fillText(roleLabel, pos.x, pos.y + 4);
    }
}
...
function getRoleColor(role, alpha = 1) {
    const colors = {
        ...
        // MeshCore roles
        'COMPANION': `rgba(52, 152, 219, ${alpha})`,  // Blue - like client
        'ROOM_SERVER': `rgba(155, 89, 182, ${alpha})`  // Purple - special server
    };
...
// MeshCore route cache display
function updateRouteCacheDisplay(count) {
    const countSpan = document.getElementById('cached-routes-count');
    if (countSpan) {
